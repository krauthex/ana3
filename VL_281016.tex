...
\begin{document}
	...
	\subsection*{Lebesgue-Maß}
		...
		\begin{proof}[Beweis von Lemma 31]
			...
			\begin{enumerate}[(a)]
				...
				\item Offenbar gilt $\lambda(\es) = 0$. Für $\sigma$-Additivität ist
					...
					\]
					\footnotetext{$\lambda{\left(\bigcup_{j=1}^k B_j\right)}
								= \sum_{k\in\N}^k \lambda(B_j)$
		...
		\begin{defin}
			Seien $(X,\Sigma_X)$, $(Y,\Sigma_Y)$ Messräume. \\
			Eine Funktion $f \colon X \to Y$ heipt \textbf{messbar} (eigentlich
			\textbf{$\boldsymbol{\Sigma_X}$-$\boldsymbol{\Sigma_Y}$-messbar}), falls \\
