...
\begin{document}
	...
	\begin{cor}[Globaler Umkehrsatz]
		...
		\begin{proof}
			Nach Vorraussetztung ist $f \colon \Omega \to \mc{W}$ bijektiv und $C^1$. \emph{Satz 2} impliziert nun, dass D$f(x) \: \forall x \in \Omega$ invertierbar ist und dass 
			...
			Sei $f \colon X \to Y$, dann ist Graph $f = \{(x,f(x)) \mid x \in X \} \sbs X \times Y$. Beispielweise können wir die Sphäre $ \mathds{S}^2 = \{(x_1, x_2, y) \in \R^3 \mid \underbrace{x_1^2 + x_2^2 + y^2 =1}_{\mathclap{f(x_1,x_2,y) = x_1^2 + x_2^2 + y^2 - 1}} \} = f^{-1}(\{0\})$ für $y>0$ als Graph der Funktion $g(x_1,x_2) = \sqrt{1- x_1^2 - x_2^2}, \: (x_1,x_2) \in B_1(0)\sbs \R^2$ darstellen.
		\end{proof}
	\end{cor}
