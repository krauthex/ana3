...
\begin{document}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= \int_{\R^n} \widehat{f(\bcdot + x)}(p) \phi(\varepsilon p) \md \lambda^n(p) \uarrow{\text{Lemma 3(v)}}{=} \int_{\R^n} f(p+x) \underbrace{\widehat{\phi( \varepsilon \bcdot)}(p)}_{\phi_{\varepsilon}(p)} \md \lambda^n(p)
		\end{align*}
	...
	\begin{lem}[Plancherel-Identität]
	...
	\begin{proof}
		...
		\begin{align*}
		\int_{\R^n} \abs{\wh{f}(p)}^2 \md \lambda^n(p) &= \frac{1}{(2 \pi)^{\frac{n}{2}}} \int_{\R^n} \int_{ \R^n} f(x) \underbrace{\e^{-ipx} \overline{\wh{f}(p)}}_{=\overline{ \e^{ipx} \wh{f}(p)}} \md \lambda^n(x) \md \lambda^n(p) \\
		&= \frac{1}{(2 \pi)^{\frac{n}{2}}} \int_{ \R^n} f(x)  \underbrace{\overline{ \int_{ \R^n} e^{ipx} \wh{f}(p) \md \lambda^n(p)}}_{= (2 \pi)^\frac{n}{2} \wc{(\wh{f})}(x) = (2 \pi)^{\frac{n}{2}} f(x)} \\
		&= \int_{\R^n} \underbrace{f(x) \overline{f(x)}}_{\abs{f(x)}^2} \md \lambda^n (x)\\
	     \text{Insbesondere } 
	...
	\end{proof}
		...
		\section[Fortsetzbarkeit auf \texorpdfstring{$L^2$}{L2}]{Fortsetzbarkeit auf \texorpdfstring{$\boldsymbol{L^2}$}{L2}}
		\begin{theorem}[Fortsetzung linearer Abbildungen]
			...
			\begin{proof}
				...
				\]
				 und $(A_k)_{k\in \N} \sbs Y$ eine Cauchyfolge und diese besitzt einen eindeutigen Grenzwert $\tilde{A}x_0 \ceq \lim_{k \to \infty} Ax_j$, sofern $x_0 = \lim_{k \to \infty} x_k \in X$ existiert.
